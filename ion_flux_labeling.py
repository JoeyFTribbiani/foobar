...
#Use math trick to solve this problem, time complexity O(len(q)*h), space complexity O(len(q))
...
def answer(h, q):
    ...
    for node in q:
        if node == 2**h - 1:
            ...
            curnode = node
            # the following is derived from the observation
            while curnode not in hashTable:
                # node is left child node
                if 2**int(math.log(curnode+1,2)) == curnode + 1:
                    hashTable[node] = curnode + 1
                    break
                # node is right child node
                elif 2**int(math.log(curnode+2,2)) == curnode + 2:
                    hashTable[node] = 1
                    break
                curnode = curnode - 2**int(math.log(curnode,2)) + 1
            if node not in hashTable:
                hashTable[node] = hashTable[curnode]
            res += node + hashTable[node],
    return res